...
EntityMage = ig.Entity.extend({
	...
	init: function(x, y, settings) {
		...
		this.fireballTimer = new ig.Timer(2);
	...
	update: function() {
		...
		if(ig.game.getEntitiesByType(EntityPlayer).length > 0) {
			...
			if(this.fireballTimer.delta() > 0) {
				ig.game.spawnEntity(EntityFireball, this.pos.x, this.pos.y, {vel: {x: Math.cos(this.angleTo(player)) * 70, y: Math.sin(this.angleTo(player)) * 70}});
				this.fireballTimer.set(1);
			}
