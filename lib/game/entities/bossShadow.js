...
EntityBossShadow = ig.Entity.extend({
	...
	init: function(x, y, settings) {
		...
		this.vulnrableTimer = new ig.Timer(0);
		this.startChargeTimer = new ig.Timer(0);
	},

	check: function(other) {
		if(this.currentAnim == this.anims.melee && other instanceof EntityPlayer) {
	...
	update: function() {
		if (this.inGame) {
			...
			if(this.isVulnerable && this.vulnrableTimer.delta() > 0) {
				this.isVulnerable = false;
			}
	...
	receiveDamage: function(amount, from) {
		if(this.invulnerableTimer.delta() > 0) {
			if(this.isVulnerable) {
				...
				this.parent(Math.floor((amount+2.5)/5), from);
			}
	...
	handleMovement: function(player) {
		...
		if(this.currentAnim.frame == 4 && this.bossState == 1 && this.startChargeTimer.delta() > 0){
			this.isCharging = true;
			this.retreatTimer.set((Math.random()*3.5)+0.5);
			this.retreatTimer.pause();
			this.bossState = 2;
			this.chargeTimer.set(5);
		}
		
		if(this.isCharging) {
				this.vel.x = 1 * Math.cos(this.angleTo(player)) * 120;
				this.vel.y = 1 * Math.sin(this.angleTo(player)) * 120;
				if(this.chargeTimer.delta() > 0) {
					this.vel.x = 0;
					this.vel.y = 0;
					this.isCharging = false;
		...
		if(this.health <= 2500 && this.bossState < 5){
			this.vel.x = 0;
			this.vel.y = 0;
