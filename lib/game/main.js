...
.requires(
	...
	//Player Sword
	'game.entities.swordUD',
	'game.entities.swordLR',

	//Player Fist
	'game.entities.fistUD',
	'game.entities.fistLR',
...
MyGame = ig.Game.extend({
	...
	init: function() {
		...
		this.myDirector = new ig.Director(this, [LevelTest, LevelTest2, LevelTest3]);
	...
	initializeControls: function() {
		...
		ig.input.bind( ig.KEY.F, 'fist');
		ig.input.bind( ig.KEY.P, 'pause');
		ig.input.bind( ig.KEY.S, 'sword');
		ig.input.bind( ig.KEY.G, 'grapplinghook');
		ig.input.bind( ig.KEY.V, 'magiccloak');
		ig.input.bind( ig.KEY.E, 'bow');
	},
	...
	normalEnemyItemDrop: function(posx, posy) {
		if(Math.floor(Math.random()*11) > 6) {
			//spawn health pack or gold
			this.spawnEntity(EntityHealthkit, posx+10, posy+10);
		}
	},
...
StartScreen = ig.Game.extend({
	...
	draw: function() {
		...
		this.instructText.draw('Press Spacebar to Begin Game NOW!', x+40, y, ig.Font.ALIGN.CENTER);
	}
});
